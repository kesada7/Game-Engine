...
- Deprecated DebugText, use cout and cerr instead
- Removed performance profiler as the one in Visual Studio does the same
- Enabled rendering text with an outline
...
- Windows 7 is no longer supported so removed specific dinput dll fix
...
namespace HAPISPACE {
	...
	public:
		...
		// Returns entered character as text (takes into account localisation and modifier keys etc).
		...
		virtual bool PlayStreamedMedia(const std::string &filename, const HAPI_TStreamedMediaOptions& options, int &instanceId) = 0;

		// Restarts streamed media that has already been played (more efficient than reloading it)
		virtual bool PlayStreamedMedia(int instanceId) = 0;
		...
		virtual bool RenderText(int x, int y, const HAPI_TColour &fillColour, const std::string& text,
		...
		virtual bool RenderText(int x, int y, const HAPI_TColour &fillColour, const HAPI_TColour &outlineColour,
		...
		// Means to delete the data you must call: _aligned_free(data);
		...
		// Useful as a root for then finding data files etc.
